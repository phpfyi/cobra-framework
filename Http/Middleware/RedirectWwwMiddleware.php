 ...
 * @since     1.0.0
 */
class RedirectWwwMiddleware extends Middleware
{
    /**
    ...
    {
        if ($request->getUri()->isWwwHostname() && env('FORCE_WWW') !== true) {
            return http_redirect_response(
                $handler,
                $request->getUri()->withHost(
                    substr($request->getUri()->getHost(), 4)
                )
            );
        }
